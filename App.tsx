...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage } from './services/geminiService.ts';
...
import { UndoIcon, RedoIcon, EyeIcon } from './components/icons.tsx';
...
type Tab = 'retouch' | 'removeBg' | 'adjust' | 'filters' | 'crop';
...
const App: React.FC = () => {
  ...
  const handleApplyAdjustment = useCallback(async (adjustmentPrompt: string) => {
    ...
    const handleRemoveBackground = useCallback(async () => {
        ...
        try {
            const backgroundRemovalPrompt = "Perfectly cut out the main subject and place it on a transparent background.";
            const resultImageUrl = await generateAdjustedImage(currentImage, backgroundRemovalPrompt);
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'removeBg', 'crop', 'adjust', 'filters'] as Tab[]).map(tab => (
                 <button
                    key={tab}
